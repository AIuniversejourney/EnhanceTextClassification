...
if __name__ == '__main__':
    ...
    def log_training_results(trainer):
        ...
        for batch in IDX_LOADER_TEST:
            y_pred, y_true = test_step(None, batch)  # Call test_step
            y_preds.append(y_pred)
            y_trues.append(y_true)
        y_preds = th.cat(y_preds, dim=0).cpu().numpy()
        y_trues = th.cat(y_trues, dim=0).cpu().numpy()

        test_precision = precision_score(y_trues, y_preds.argmax(axis=1), average='weighted', zero_division=0)
        test_recall = recall_score(y_trues, y_preds.argmax(axis=1), average='weighted',zero_division=0)
        test_f1 = f1_score(y_trues, y_preds.argmax(axis=1), average='weighted', zero_division=0)
